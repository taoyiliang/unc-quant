...
public:
  TwoDProblem(TwoDMesh &mesh, Epetra_Comm &comm );
  ...
  double computeCurrent(int icell, int g, const Epetra_Vector * soln);
  double computeCurrentX(int icell, int g, const Epetra_Vector * soln);
  double computeCurrentY(int icell, int g, const Epetra_Vector * soln);
  ...
  double computeNonlinearKeff(const Epetra_Vector &soln);

  double computeFissionSourceStrength(double keff, const Epetra_Vector *soln);
  
  void copyOldSolution(Epetra_Vector &soln);

  void setBCs(int left, int right, int bottom, int top);

  void setNCAFlag(bool flag){_nca_flag=flag;}

  void AppendOutVals(int,int,int,double,std::string);
