...
for h in hs[1:]:
  ...
  for pt in runs[h].keys():
    if pt=='k':
      ...
      err1[h]=max(err1[h],abs(runs[h][pt][1]-runs[ref][pt][1]))
      err2[h]=max(err2[h],abs(runs[h][pt][2]-runs[ref][pt][2]))
...
def addSlopes(x,y):
  for i in range(len(x)-1):
    ...
    plt.text(avx,pty,'%1.2e' %slope,ha='center')
