...
print 'Variables:',histories['varNames']
while not sampler.converged:# and total_runs<maxSamples:
  ...
  print '   soln',histories['soln'][-1]
  #print 'solution:',histories['soln'][-1]

#build backend
#def cprToActual(y,x):
#  return x*np.exp(-x*x)+np.exp(-y*y)
  #return x*x + x*y + y*y
  #return x+y
def cprToActual(x):
  #return np.exp(-x*x)
...
for beType in backendTypes:
  if beType in ['plot2D']:
    ...
    if len(histories['vars'])==1:
      print 'Polynomial:'
      print backend.OneDPrintPoly(histories)
      print ''
