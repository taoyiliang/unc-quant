...
def parBasicSparse(nump,N,L,indexset,quadrule,varlist,verbose={}):
...
class CoeffMaker:
  ...
  def parMakeCoeffs(self,nump,N,indexset):
    procs=[]
    done=False
    allStarted=False
    NI=len(indexset)
    ...
    while not done:
      for p,proc in enumerate(procs):
        if not proc.is_alive():
          ...
          while not self.coefq.empty():
            ...
            c+=cofs
          del procs[p]
          print '  ...finished jx %i/%i (%i pct)...\r'\
                   %(numdone,numtodo,100*float(numdone)/float(numtodo)),
      ...
      while len(procs)<nump and not allStarted:
        try:
          ...
          procs.append(multiprocessing.Process(target=self.makeCoeffBatch,
                                               args=[jxs,NI,iset]))
          procs[-1].start()
